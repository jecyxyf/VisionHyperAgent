...
Linux 部署包为 `bin/VisionHyperAgent-linux-x64.zip`。解压后运行 `./VisionHyperAgent`。
...
或者执行 `./build.sh` 构建并在 `bin/` 中生成 Linux / Windows 绿色程序压缩包；不再输出裸 exe。
...
bin/                        Linux / Windows 压缩包、本机私有配置和忽略的测试证据
```
