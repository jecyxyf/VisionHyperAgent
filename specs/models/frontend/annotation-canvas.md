...
本文件同时是 `annotation-canvas` 的 API 契约和 2119 需求来源。需求约束归一化几何、编辑历史、验证提示和资源释放。

## Requirements

### 1: 实例遮罩编辑

1. 画布 MUST 输出归一化坐标和正确的实例版本 [manual]
2. 画布 MUST 对越界、自交和点数不足的几何结果给出验证警告 [manual]
3. 画布 MUST 保持撤销和重做顺序，不得改变未关联图片的文档 [manual]
4. 画布销毁后 MUST NOT 再向页面发送编辑或渲染事件 [manual]
...
| 2119 ID | 验证行为 | 当前证据 |
| --- | --- | --- |
| `annotation-canvas.1.1` | 输出归一化几何 | 规划验收；实现后补坐标测试 |
| `annotation-canvas.1.2` | 非法几何给出警告 | 规划验收；实现后补边界测试 |
| `annotation-canvas.1.3` | undo/redo 顺序稳定 | 规划验收；实现后补历史测试 |
| `annotation-canvas.1.4` | destroy 后不再发事件 | 规划验收；实现后补资源释放测试 |

测试使用 `// 2119: annotation-canvas.1.1` 标记；快照只能辅助，不能替代几何断言。
