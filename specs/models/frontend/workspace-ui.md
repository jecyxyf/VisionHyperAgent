...
本文件同时是 `workspace-ui` 的 API 契约和 2119 需求来源。需求约束页面状态、命令确认、事件同步和网页生命周期。

## Requirements

### 1: 工作区交互与同步

1. 工作区 MUST 在后端快照或事件确认前保持命令处于提交中状态 [manual]
2. 工作区 MUST 在请求失败时保留用户输入草稿 [manual]
3. 工作区 MUST 在事件序号出现缺口时请求后端快照同步 [manual]
4. 工作区销毁时 MUST NOT 取消后端任务 [manual]
...
| 2119 ID | 验证行为 | 当前证据 |
| --- | --- | --- |
| `workspace-ui.1.1` | 命令等待后端确认 | 规划验收；实现后补组件集成测试 |
| `workspace-ui.1.2` | 失败保留草稿 | 规划验收；实现后补失败交互测试 |
| `workspace-ui.1.3` | 事件缺口触发同步 | 规划验收；实现后补断线恢复测试 |
| `workspace-ui.1.4` | 页面销毁不取消后台任务 | 规划验收；实现后补 Playwright 测试 |

测试使用 `// 2119: workspace-ui.1.1` 标记，并断言用户可见状态而不是快照数量。
