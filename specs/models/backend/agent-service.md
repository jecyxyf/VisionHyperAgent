...
本文件同时是 `agent-service` 的 API 契约和 2119 需求来源。需求约束会话、计划确认、上下文隔离和配置重载等调用方可观察行为。当前需求处于规划与验收基线阶段，暂以人工验收标记，进入实现后再绑定自动化测试。

## Requirements

### 1: 会话、计划与配置

1. Agent 服务 MUST 在每个发送回合结束时产生完成、停止或失败中的一种终态事件 [manual]
2. Agent 服务 MUST NOT 在阶段计划未得到用户确认时创建或修改业务数据 [manual]
3. Agent 服务 MUST 从发送给模型服务的上下文中排除 API Key、原图内容和模型权重 [manual]
4. Agent 服务 MUST 使配置重载只影响新建会话，活动会话继续使用创建时的配置快照 [manual]
...
| 2119 ID | 验证行为 | 当前证据 |
| --- | --- | --- |
| `agent-service.1.1` | 回合最终产生唯一终态事件 | 规划验收；实现后补流式集成测试 |
| `agent-service.1.2` | 未确认计划不得修改业务数据 | 规划验收；实现后补拒绝路径测试 |
| `agent-service.1.3` | 上下文不得包含敏感数据 | 规划验收；实现后补脱敏测试 |
| `agent-service.1.4` | 配置重载不影响活动会话快照 | 规划验收；实现后补并发重载测试 |

测试标记使用 `// 2119: agent-service.1.1`。Codex mock 只能替代外部传输，不能替代计划确认和终态断言。
