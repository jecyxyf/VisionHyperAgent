...
本文件同时是 `api-gateway` 的 API 契约和 2119 需求来源。需求约束 HTTP、WebSocket、上传、错误映射和关闭边界。

## Requirements

### 1: 协议边界

1. 网关 MUST 为每个响应提供可关联的 request_id 和稳定错误码 [manual]
2. 网关 MUST 拒绝非法 JSON、超限上传和越出允许根目录的路径 [manual]
3. 网关 MUST 在 WebSocket 事件游标失效时要求客户端同步快照 [manual]
4. 网关 MUST 在关闭时停止接收新请求、关闭订阅并保持后台任务原有终态 [manual]
...
| 2119 ID | 验证行为 | 当前证据 |
| --- | --- | --- |
| `api-gateway.1.1` | 响应包含 request_id 和稳定错误码 | 规划验收；实现后补路由集成测试 |
| `api-gateway.1.2` | 非法输入、超限上传和越界路径被拒绝 | 规划验收；实现后补边界测试 |
| `api-gateway.1.3` | 事件游标失效触发同步 | 规划验收；实现后补断线重连测试 |
| `api-gateway.1.4` | 关闭不伪造任务终态 | 规划验收；实现后补生命周期测试 |

测试应在真实路由和序列化边界执行，并使用 `// 2119: api-gateway.1.1` 标记。
