...
本文件同时是 `project-service` 的 API 契约和 2119 需求来源。需求约束项目、图片、规则、标注版本和数据集来源。

## Requirements

### 1: 项目数据与版本

1. 项目服务 MUST 在图片导入后保持原始图片内容不变 [manual]
2. 项目服务 MUST 只为规则和几何均有效的草稿创建不可变标注确认版本 [manual]
3. 项目服务 MUST 只把已确认标注纳入数据集版本 [manual]
4. 项目服务 MUST 在数据集版本中保存图片、标注、规则和哈希来源 [manual]
...
| 2119 ID | 验证行为 | 当前证据 |
| --- | --- | --- |
| `project-service.1.1` | 原图导入后不可变 | 规划验收；实现后补文件完整性测试 |
| `project-service.1.2` | 非法标注不能确认 | 规划验收；实现后补规则和几何测试 |
| `project-service.1.3` | 未确认标注不能进入数据集 | 规划验收；实现后补数据集集成测试 |
| `project-service.1.4` | 数据集来源可追溯 | 规划验收；实现后补哈希和版本测试 |

测试使用 `// 2119: project-service.1.1` 标记，并覆盖部分成功和事务回滚。
