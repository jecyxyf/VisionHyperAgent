...
本文件同时是 `task-runtime` 的 API 契约和 2119 需求来源。需求约束任务状态机、事件顺序、取消、恢复和进程监督。

## Requirements

### 1: 任务状态与恢复

1. 任务运行时 MUST 只允许定义过的状态转移进入终态 [manual]
2. 任务运行时 MUST 在取消超时后回收 Worker 进程树并记录最终原因 [manual]
3. 任务运行时 MUST 将无法由完成凭证证明的运行中任务标记为 interrupted [manual]
4. 任务运行时 MUST 在事件游标缺口时要求同步并保持网页断开前的任务状态 [manual]
...
| 2119 ID | 验证行为 | 当前证据 |
| --- | --- | --- |
| `task-runtime.1.1` | 非法状态转移被拒绝 | 规划验收；实现后补状态机测试 |
| `task-runtime.1.2` | 取消超时会回收进程 | 规划验收；实现后补受控 Worker 测试 |
| `task-runtime.1.3` | 无凭证任务恢复为 interrupted | 规划验收；实现后补启动恢复测试 |
| `task-runtime.1.4` | 事件缺口同步且断线不改状态 | 规划验收；实现后补事件集成测试 |

测试使用 `// 2119: task-runtime.1.1` 标记，每个状态和拒绝路径都要有独立断言。
