...
- specs/：使用说明、架构说明、示例配置和测试报告。
- specs/plans/：临时开发计划文档及需求设计进度记录。
...
4. 用户文档、示例配置和测试报告放 specs/；不要把临时报告散落在仓库根目录。
...
8. project-requirement 新生成的正式文档放 specs/；模块 API 使用 specs/models/<架构层名称>/<模块名>.md，不增加同名模块子目录。已有历史文档不因本规则自动迁移。
