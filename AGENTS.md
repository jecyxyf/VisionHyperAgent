...
当前已完成工程骨架和可独立预览的 Slint 界面初版：`src/view/main.slint` 为入口，`theme.slint` 统一主题，`components/` 与 `pages/` 组织组件和页面。导航固定为“运行；模型 → 标注、训练；设置；关于”，启动进入运行页，不保留首页。“模型”文字进入独立模型库，右侧箭头只折叠子导航，不切换当前页面；目录扫描尚未接入。使用绚彩多色渐变、半透明磨砂面板和顶部小图标工具栏，避免退回灰白保守配色；界面暂不显示“实例分割”字样，但业务范围不变。不在界面常驻显示开发说明、版本号或重复引导；操作错误按需提示，不伪装成功。全局聊天位于页面条件分支之外，切页不能清空输入或训练参数草稿；Agent 标题栏只保留清除记录按钮，清除不影响输入草稿。Rust 宿主仍仅输出终端提示，尚未连接 Slint 或业务服务。
