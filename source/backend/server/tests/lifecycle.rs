...
use vha_server::{codex_config::CodexSettings, http_server};
...
fn config(root: &Scratch, exe: PathBuf, port: u16) -> CodexSettings {
    let text=toml::to_string(&json!({"codex_path":exe,"port":port,"base_url":"http://127.0.0.1:1/v1","model":"lifecycle-fixture","api_key":"NOT_A_REAL_PROVIDER_KEY"})).unwrap();
    CodexSettings::from_toml(&root.0, &text).unwrap()
}
fn free_port() -> u16 {
...
async fn occupied_codex_port_is_not_connected_to_or_killed() {
    ...
    let server =
        http_server::start_with_codex("127.0.0.1:0".parse().unwrap(), Ok(settings)).unwrap();
...
async fn unexpected_native_child_death_keeps_http_available_and_reports_failure() {
    ...
    assert_eq!(after["activeTurns"], json!({}));
    assert_eq!(
        reqwest::get(format!("{url}/api/health"))
            .await
            .unwrap()
