...
def render(name, page, width, height, extra=''):
    ...
    source.write_text(
        'import { MainWindow } from "../../../../src/view/main.slint";\n'
        'import { ViewPage } from "../../../../src/view/theme.slint";\n'
...
def main():
    ...
    chat = (ROOT / 'src/view/components/global-chat.slint').read_text(encoding='utf-8')
    assert '清除聊天记录' in chat
    assert 'Agent 连接设置' not in chat
    assert 'text: "未连接"' not in chat
    inference = (ROOT / 'src/view/pages/inference.slint').read_text(encoding='utf-8')
    assert 'label: "网络触发配置"' not in inference
    assert 'text: "未就绪"' not in inference
    run('--check', ROOT / 'src/view/main.slint')
    for width, height in [(1440, 900), (1120, 720)]:
        for page in ['inference', 'models', 'annotation', 'training', 'settings', 'about']:
