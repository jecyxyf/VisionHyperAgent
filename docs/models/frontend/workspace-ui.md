...
规格应覆盖主工作区的可观察状态、命令、事件同步和网页生命周期：

| 行为 | 必须验证 | 反例测试 |
| --- | --- | --- |
| 操作反馈 | 命令成功后等待后端快照或事件确认 | 请求失败不得清空用户草稿 |
| 同步恢复 | 事件缺口触发快照同步，刷新后恢复项目和任务 | 过期事件不得覆盖新状态 |
| Agent 交互 | 计划确认、停止和失败提示与后端事件一致 | 前端不得绕过确认按钮直接执行 |
| 断开处理 | WebSocket 断开保留后台任务和输入 | 页面销毁不得发送取消任务 |

Playwright 或组件测试必须断言用户可见状态和 API 边界；不以渲染快照数量代替行为证据。
