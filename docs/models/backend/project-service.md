...
规格应覆盖项目、图片、规则、标注版本和数据集来源：

| 行为 | 必须验证 | 反例测试 |
| --- | --- | --- |
| 图片导入 | 合法图片逐项保存并报告成功/失败，原图导入后不可变 | 损坏、重复、超限文件不得污染项目 |
| 标注确认 | 草稿仅在规则和几何均有效时生成不可变确认版本 | 无规则、越界坐标和未知类别必须拒绝 |
| 数据集生成 | 只收集已确认标注并记录规则、图片和哈希来源 | 空数据集或未确认图片不得生成训练输入 |
| 恢复与快照 | 读取快照不会暴露半提交数据 | 事务失败后不得出现孤立确认记录 |

测试使用临时项目目录和真实存储边界，正向用例与回滚、部分成功用例分别标注 REQ-ID。
