...
规格应覆盖启动顺序、浏览器入口、托盘、端口、子进程和退出清理：

| 行为 | 必须验证 | 反例测试 |
| --- | --- | --- |
| 启动 | 日志、配置、服务、Agent/Worker 监督和浏览器入口按约定完成 | 任一依赖启动失败时给出可诊断错误并退出清理 |
| 浏览器断开 | 标签页关闭只断开前端连接，宿主和后台任务继续 | 不能因 WebSocket 断开误杀任务 |
| 托盘退出 | 停止新任务、回收 Codex/Worker 和进程树 | 子进程残留或重复回收必须被报告 |
| 平台差异 | Windows/Linux 使用同一协议并保留退出码 | 端口冲突、权限失败和浏览器启动失败有明确结果 |

平台测试应在真实宿主进程边界执行；mock 只能用于不可用的系统通知，不能替代进程回收检查。
