...
规格应覆盖训练、评估、推理任务的参数校验、资源策略、模型状态和结果登记：

| 行为 | 必须验证 | 反例测试 |
| --- | --- | --- |
| 任务创建 | 只有有效 manifest、模型和已确认参数才能入队 | 缺少标注、越界参数或未确认计划必须拒绝 |
| 资源策略 | GPU/显存不足时明确降级或拒绝，不伪造可运行 | 无 GPU、显存不足和设备不可用分别验证 |
| 结果登记 | 成功、失败、取消都关联任务、模型和产物 | 缺少完成凭证不得登记成功 |
| 重试与幂等 | 相同任务输入不会产生不可解释的重复结果 | 不同参数复用同一任务 ID 必须冲突 |

测试应覆盖与 `task-runtime` 的真实协议和用户可见状态，而不是只测请求结构体。
