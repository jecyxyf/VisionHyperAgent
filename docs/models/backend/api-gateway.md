...
规格应覆盖 HTTP、WebSocket、上传、错误映射和关闭行为：

| 行为 | 必须验证 | 反例测试 |
| --- | --- | --- |
| 请求响应 | 每个响应包含可关联的 request_id 和稳定错误码 | 非法 JSON、未知路由和超大上传被拒绝 |
| 事件订阅 | 事件序号单调递增，游标失效时要求同步 | 事件缺口不能静默继续 |
| 数据边界 | 上传路径、大小和类型受限，响应不泄漏内部路径 | 路径穿越、密钥和堆栈信息不得返回 |
| 关闭 | 停止接收新请求并关闭订阅，后台任务交由运行时处理 | 关闭网关不得伪造任务成功或取消 |

测试使用真实路由和序列化边界，至少包含一个断线重连和一个脱敏错误断言，并在测试附近标注 REQ-ID。
